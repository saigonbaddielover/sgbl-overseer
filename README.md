...
| `send [--yes] [--force] <target> <msg\|->` | **Agent (Claude/Codex).** Place + submit, confirm the turn started (so a following `wait` doesn't race), don't wait for the reply. If the agent is **compacting** (or otherwise mid-turn), the message is **queued** — `send` says so and exits 0; it was accepted and runs when the agent frees up, so `wait` for it. |
