...
- **Survey first, then remediate.** `overseer hosts` (inventory from `$OVERSEER_HOSTS`, `~/.ssh/config`,
  ...
  sudo; `--dry-run` to preview), then re-survey; `DRIVE=win*` → use the `win*` commands (admin +
