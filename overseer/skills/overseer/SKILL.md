...
| `winsh <host> <command> [timeout]` | **Remote (SSH), Windows target.** Run one command line in the broker's **pwsh** child, wait via a unique sentinel, print output + exit code. The Windows peer of `sh` (needs `winbroker <host> pwsh`). | **SIDE EFFECT** |
| `winchat <host> <prompt> [timeout]` | **Remote (SSH), Windows target.** Send a prompt to the broker's **claude/codex** child, submit, then wait for the turn by reading the agent's on-disk transcript with the *same* `transcript.sh` readers (run on the file fetched back over ssh), and print the reply. The Windows peer of `chat` (needs `winbroker <host> claude\|codex`). | **SIDE EFFECT** |
