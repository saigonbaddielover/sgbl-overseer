...
| `hosts [--list] [-t secs]` | **Remote (SSH), fleet survey.** One line per host — `ONLINE OS SSH DRIVE` — so you can see which machines are reachable/drivable before an `on`/`win*`. Inventory of ssh targets comes from `$OVERSEER_HOSTS`, else `$XDG_CONFIG_HOME/overseer/hosts`, else the non-wildcard `Host` entries in `~/.ssh/config` (each carries the user + key a bare Tailscale IP lacks). Probes **live and in parallel**: `SSH` = `ok`/`deny`/`unreach`, `OS` = `linux`/`windows`/`macos`, `DRIVE` = `yes` (Linux w/ `tmux`+`jq`), `no:tmux`/`no:jq`, or `win*`. `ONLINE` from `tailscale status` if present. Stateless — no cred/health store (ssh keys stay the credential; reachability is recomputed each run, never cached). `--list` = inventory only, no ssh; `-t` = per-host connect timeout (default 6s). | read-only (ssh probe: `uname`, `command -v`) |
