...
| `chat [--yes] <target> <message\|-> [timeout]` | **Agent pane (Claude or Codex).** Send the message, **wait for the turn to finish**, then print the reply. The human round-trip. If the agent is **busy (mid-turn) or compacting**, the message is **queued** and `chat` waits for *its own* queued turn to complete — not the running one. The reply it prints is bound to *your* message (the first completed turn after your prompt), so it is never the running turn's reply, nor a later `task-notification`/background turn's if the agent is itself orchestrating. If the agent stops at an interactive prompt (permission / plan / select menu) it returns that question + how to answer instead of hanging. | **SIDE EFFECT** |
| `send [--yes] <target> <message\|->` | **Agent pane (Claude or Codex).** Place + submit the message, then confirm the turn actually started before returning (so a following `wait`/`read` doesn't race) — but do **not** wait for the reply (use `chat` for that). If the agent is **busy (mid-turn) or compacting its context**, Claude **queues** the message: `send` reports `QUEUED` and exits 0 — it *was* received and runs when the agent frees up, so tell the user it is queued (not lost) and `wait` for the reply. Do **not** resend. | **SIDE EFFECT** |
...
- "How are all my agents doing / act on all of them" → `fleet status` (one line per pane: idle/busy/
  awaiting), `fleet read`/`fleet wait` to fan those out; add `--hosts` to sweep every machine, not just
  this one. `fleet send`/`fleet chat` only when the user explicitly asks to broadcast the same message
