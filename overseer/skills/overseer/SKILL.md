...
| `chat [--yes] <target> <message\|-> [timeout]` | **Agent pane (Claude or Codex).** Send the message, **wait for the turn to finish**, then print the reply. The human round-trip. If the agent is **busy (mid-turn) or compacting**, the message is **queued** and `chat` waits for *its own* queued turn to complete — not the running one — so the reply it prints is the answer to *your* message, never the current turn's. If the agent stops at an interactive prompt (permission / plan / select menu) it returns that question + how to answer instead of hanging. | **SIDE EFFECT** |
