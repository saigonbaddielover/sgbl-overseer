...
Check 'broker requires an AUTH handshake' $true ($broker -match '\[string\]::Equals\(\$auth, \$want, \[StringComparison\]::Ordinal\)')
Check 'broker compares the token length-first, case-sensitively' $true ($broker -match '\$auth\.Length -ne \$want\.Length')
Check 'broker demands a first pipe instance' $true ($broker -match 'PipeOptions\]::FirstPipeInstance')
Check 'broker writes its transcript claim to the state file, not the secret' $true (($broker -match 'Set-Content -LiteralPath \$StatePath') -and -not ($broker -match 'ConvertTo-Json -Compress \| Set-Content -LiteralPath \$Config '))
Check 'broker validates the resolved transcript path' $true ($broker -match 'Test-TranscriptPath')
...
Check 'client connects with an anonymous impersonation level' $true ($client -match 'TokenImpersonationLevel\]::Anonymous')
...
Check 'client validates the broker name' $true ($client -match 'overseer-broker\(\?:-\[0-9A-Za-z_-\]\+\)\?\\z')
Check 'client skips state files when listing brokers' $true ($client -match '\\\.state\\\.json')
...
Check 'launcher restricts the descriptor ACL' $true ($launch -match 'Set-FileAcl \$configPath \$cu ''ReadAndExecute''')
Check 'launcher gives the console user a separate writable state file' $true ($launch -match 'Set-FileAcl \$statePath \$cu ''Modify''')
Check 'launcher hardens each staged payload file' $true ($launch -match "foreach \(\`$pf in")
Check 'launcher drops Authenticated Users from the shared dirs' $false ($launch -match 'Authenticated Users')
...
Check 'launcher still defaults to claude and codex' $true (($launch -match "else \{ 'claude' \}") -and ($launch -match "else \{ 'codex' \}"))
